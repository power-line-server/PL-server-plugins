 ...
 * (ext/vote) Vote System includes: changeMap gameOver skipWave text | 投票系统(换图,投降,跳波,自定义文本投票)
...
listenPacket2ServerAsync<ConnectPacket> { con, packet ->
    ...
    if (event.cancelled && !con.kicked)
        con.kick("{tr module.kick.rejected}".with("reason" to (event.reason ?: "")).toString())
    !event.cancelled
}
