...
command("saveSnap", "{tr command.saveSnap.desc}".with()) {
    type = CommandType.Server
    body {
        val img = MapRenderer.img ?: returnReply("{tr mapSnap.reply.mapNotLoaded}".with())
        ...
        reply("{tr mapSnap.reply.snapshotSaved}".with("file" to file))
    }
}